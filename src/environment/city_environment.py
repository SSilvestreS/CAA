...
class CityEnvironment:
    ...
    def __init__(self, city_name: str = "Cidade Inteligente", 
                 city_size: tuple = (100, 100), **kwargs):
    ...
    async def initialize_city(self, num_citizens: int = 100, num_businesses: int = 20, 
                            num_infrastructure: int = 10) -> None:
    ...
    async def _update_city_metrics(self) -> None:
        ...
        # Saúde ambiental
        environmental_health = 1.0 - sum(
            business.get_business_metrics().get('environmental_impact', 0) 
            for business in self.businesses
        ) / len(self.businesses) if self.businesses else 1.0
        
        # Satisfação cidadã
        citizen_satisfaction = np.mean([citizen.state.satisfaction for citizen in self.citizens])
        
        # Saúde econômica
        economic_health = np.mean([business.get_business_metrics().get('profit_margin', 0) for business in self.businesses])
        
        # Saúde da infraestrutura
        infrastructure_health = np.mean([infra.get_infrastructure_metrics().get('system_health', 0) for infra in self.infrastructure])
