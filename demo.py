...
class SmartCityDemo:
    ...
    async def initialize_demo(self):
        ...
        await self.environment.initialize_city(num_citizens=80, num_businesses=15, num_infrastructure=8)
    ...
    async def run_baseline_simulation(self, duration: int = 50):
        ...
        for i in range(duration):
            ...
            if i % 10 == 0:
                ...
                print(
                    f"  Ciclo {i+1}: População={status['agents_count']['citizens']}, "
    ...
    async def run_learning_demo(self):
        ...
        self.learning_system.share_knowledge(
            "citizen_1", "estratégia_otimista", 0.85, {"contexto": "alta_demanda", "setor": "transporte"}
        )
        self.learning_system.share_knowledge(
            "citizen_2", "estratégia_conservadora", 0.75, {"contexto": "baixa_demanda", "setor": "energia"}
        )
    ...
    async def run_comparative_analysis(self):
        ...
        best_scenario = max(results.items(), key=lambda x: x[1].get("citizen_satisfaction_change", 0))
        worst_scenario = min(results.items(), key=lambda x: x[1].get("citizen_satisfaction_change", 0))
