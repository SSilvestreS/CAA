...
class SmartCitySimulation:
    ...
    async def initialize(self, num_citizens: int = 100, num_businesses: int = 20, 
        ...
        try:
            if duration_hours:
                ...
                await asyncio.wait_for(
                    self.environment.start_simulation(),
...
async def main():
    ...
    parser.add_argument("--citizens", type=int, default=100, 
                       help="Número de cidadãos (padrão: 100)")
    parser.add_argument("--businesses", type=int, default=20, 
                       help="Número de empresas (padrão: 20)")
    parser.add_argument("--infrastructure", type=int, default=10, 
                       help="Número de infraestruturas (padrão: 10)")
    parser.add_argument("--duration", type=int, 
                       help="Duração da simulação em horas (padrão: indefinida)")
    parser.add_argument("--dashboard-only", action="store_true", 
                       help="Executa apenas o dashboard")
    parser.add_argument("--city-name", type=str, default="Cidade Inteligente",
                       help="Nome da cidade")
    parser.add_argument("--city-size", type=str, default="100,100",
...
def run_scenario_example():
    ...
    asyncio.run(simulation.initialize(
        num_citizens=50,
        num_businesses=10,
